use anyhow::Result;
use colored::*;
use crate::helpers::{TestContext, GitsmithRunner, assert_contains, assert_file_contains};
...
async fn test_init_new_repo(verbose: bool, keep_temp: bool) -> Result<()> {
    ...
    assert_contains(&output.stdout, "\"successes\"", "Should show successful relays")?;
...
async fn test_init_existing_repo(verbose: bool, keep_temp: bool) -> Result<()> {
    ...
    let output = runner.run_success(&[
        "init",
        "--identifier", "existing-repo",
        "--name", "Existing Repo",
        "--description", "Testing with existing repo",
...
async fn test_detect_from_git(verbose: bool, keep_temp: bool) -> Result<()> {
    ...
    assert_contains(&output.stdout, &identifier, "Should detect saved identifier")?;
    assert_contains(&output.stdout, "Detect Test", "Should detect saved name")?;
    assert_contains(&output.stdout, "Testing detection", "Should detect saved description")?;
    assert_contains(&output.stdout, "wss://relay.example.com", "Should detect saved relay")?;
