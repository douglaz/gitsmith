use anyhow::Result;
use colored::*;
use crate::helpers::{TestContext, GitsmithRunner, assert_contains};
...
async fn test_send_pr_simple(verbose: bool, keep_temp: bool) -> Result<()> {
    ...
    runner.run_success(&[
        ...
        "--nsec", &nsec,
        "--repo-path", &ctx.repo_path.to_string_lossy(),
    ])?;
    ...
    assert_contains(&output.stderr, "Generated 1 patch(es)", "Should generate patches")?;
    assert_contains(&output.stderr, "Created", "Should create events")?;
    assert_contains(&output.stderr, "✅ Pull request sent successfully", "Should send successfully")?;
...
async fn test_send_pr_with_title_description(verbose: bool, keep_temp: bool) -> Result<()> {
    ...
    runner.run_success(&[
        "init",
        "--identifier", "pr-title-test",
        "--name", "PR Title Test",
        "--description", "Testing with title/desc",
    ...
    assert_contains(&output.stderr, "Generated 2 patch(es)", "Should generate 2 patches")?;
...
async fn test_send_pr_no_commits(verbose: bool, keep_temp: bool) -> Result<()> {
    ...
    runner.run_success(&[
        "init",
        "--identifier", "pr-no-commits",
        "--name", "No Commits Test",
        "--description", "Testing with no commits to send",
        "--relay", "wss://relay.damus.io",
        "--nsec", &nsec,
        "--repo-path", &ctx.repo_path.to_string_lossy(),
    ])?;
    ...
    assert_contains(&output.stderr, "Not enough commits", "Should fail with not enough commits")?;
...
async fn test_send_pr_multiple_patches(verbose: bool, keep_temp: bool) -> Result<()> {
    ...
    runner.run_success(&[
        "init",
        "--identifier", "pr-multiple",
        "--name", "Multiple Patches Test",
        "--description", "Testing with multiple patches",
        "--relay", "wss://relay.damus.io",
        "--nsec", &nsec,
        "--repo-path", &ctx.repo_path.to_string_lossy(),
    ])?;
    ...
    let output = runner.run_success(&[
        "send",
        "--title", "Multiple commits PR",
        "--description", "This PR contains multiple patches",
        "--repo-path", &ctx.repo_path.to_string_lossy(),
        "--password", "test",
        "HEAD~5"
    ])?;
    
    assert_contains(&output.stderr, "Generated 5 patch(es)", "Should generate 5 patches")?;
    assert_contains(&output.stderr, "Created", "Should create events")?;
    assert_contains(&output.stderr, "✅", "Should succeed")?;
