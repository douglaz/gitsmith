use anyhow::Result;
use colored::*;
use crate::helpers::{TestContext, GitsmithRunner, assert_contains, assert_file_exists};
...
pub async fn run_tests(verbose: bool, keep_temp: bool) -> Result<(usize, usize)> {
    ...
    match test_account_login_with_password_arg(verbose, keep_temp).await {
        ...
        Err(e) => {
            println!("  {} test_account_login_with_password_arg: {}", "✗".red(), e);
            failed += 1;
        }
    }
...
async fn test_account_login(verbose: bool, keep_temp: bool) -> Result<()> {
    ...
    // Login with the key
    let output = runner.run_success(&["account", "login", "--nsec", &nsec, "--password", "test"])?;
    
    // Verify login success
    assert_contains(&output.stderr, "Logged in as npub", "Login should show success message")?;
...
async fn test_account_login_with_password_arg(verbose: bool, keep_temp: bool) -> Result<()> {
    ...
    assert_contains(&output.stderr, "Logged in as npub", "Should login successfully")?;
...
async fn test_account_login_with_env_var(verbose: bool, keep_temp: bool) -> Result<()> {
    ...
    assert_contains(&output.stderr, "Logged in as npub", "Should login with env password")?;
...
async fn test_account_logout(verbose: bool, keep_temp: bool) -> Result<()> {
    ...
    assert_contains(&output.stderr, "No active account", "Should fail when no account is active")?;
...
async fn test_account_export(verbose: bool, keep_temp: bool) -> Result<()> {
    ...
    assert_contains(&output.stdout, "Private key: nsec", "Should export private key")?;
...
async fn test_account_list(verbose: bool, keep_temp: bool) -> Result<()> {
    ...
    assert_contains(&output.stderr, "No accounts found", "Should show no accounts message")?;
