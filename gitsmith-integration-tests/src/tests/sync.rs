...
async fn test_list_pull_requests(verbose: bool, keep_temp: bool) -> Result<()> {
    ...
    runner.run_success(&[
        "send",
        "--title", "Test PR for listing",
        "--description", "This PR will be listed",
        "--repo-path", &ctx.repo_path.to_string_lossy(),
        "--password", "test",
        "HEAD~2"
    ])?;
...
async fn test_sync_repository(verbose: bool, keep_temp: bool) -> Result<()> {
    ...
    assert_contains(&output.stderr, "Local Git State:", "Should show local git state")?;
    assert_contains(&output.stderr, "refs/heads/master", "Should show master branch")?;
...
async fn test_sync_with_saved_config(verbose: bool, keep_temp: bool) -> Result<()> {
    ...
    runner.run_success(&[
        "init",
        "--identifier", "sync-saved-test",
        "--name", "Sync Saved Test",
        "--description", "Testing sync with saved config",
    ...
    assert_contains(&output.stderr, "Local Git State:", "Should sync with saved config")?;
    assert_contains(&output.stderr, "sync-saved-test", "Should use saved identifier")?;
...
async fn test_invalid_private_key(verbose: bool, keep_temp: bool) -> Result<()> {
    ...
    assert_contains(&output.stderr, "Invalid secret key", "Should fail with invalid key error")?;
...
async fn test_missing_relays(verbose: bool, keep_temp: bool) -> Result<()> {
    ...
    assert_contains(&output.stderr, "At least one relay is required", "Should require relays")?;
