...
impl AccountStorage {
    ...
    pub fn load(path: &Path) -> Result<Self> {
        ...
        serde_json::from_str(&data)
            .with_context(|| "Failed to parse account storage")
    }
...
pub fn login(nsec_or_hex: &str, password: &str) -> Result<()> {
    ...
    let encrypted = cipher.encrypt(&nonce, secret_key_bytes.as_ref())
...
pub fn get_active_keys(password: &str) -> Result<Keys> {
    ...
    let active_npub = storage.active_npub
        .context("No active account. Please login first")?;
    
    let account = storage.accounts.iter()
    ...
    let decrypted = cipher.decrypt(nonce, account.encrypted_nsec.as_ref())
...
pub fn list_accounts() -> Result<Vec<String>> {
    ...
    Ok(storage.accounts.iter().map(|a| {
