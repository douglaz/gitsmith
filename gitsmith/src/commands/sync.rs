...
use clap::Args;
use gitsmith_core::{get_git_state, detect_from_git};
use nostr_sdk::{Client, Filter, Kind, RelayPoolNotification, SingleLetterTag, Alphabet};
...
pub async fn handle_sync_command(args: SyncArgs) -> Result<()> {
    ...
    if !repo_announcement.relays.is_empty() {
        println!("Fetching state from {} relay(s)...", repo_announcement.relays.len());
