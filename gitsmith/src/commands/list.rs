...
use clap::Args;
use gitsmith_core::{pull_request, detect_from_git};
...
pub async fn handle_list_command(args: ListArgs) -> Result<()> {
    ...
    println!("Fetching pull requests from {} relay(s)...", repo_announcement.relays.len());
    
    // List pull requests
    let prs = pull_request::list_pull_requests(
        &repo_coordinate,
        repo_announcement.relays.clone(),
    ).await?;
